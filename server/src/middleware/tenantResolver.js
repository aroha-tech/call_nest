 ...
 * or when Host is a dev tunnel (ngrok), or when Host is listed in API_BOOTSTRAP_HOSTS,
 * we use URL path + JWT tenant (same as dev):
...
export async function tenantResolver(req, res, next) {
  ...
  if (!subdomain || subdomain === MARKETING_SUBDOMAIN) {
    return res.status(404).json({
      error:
        'This URL is not a workspace sign-in page. Use the link your organization gave you (company subdomain), or the main site.',
  ...
  try {
    ...
    if (!tenant || tenant.is_deleted) {
      return res.status(404).json({
        error: `No workspace found for "${subdomain}". Check the subdomain in your browser (for example, if your company uses "rs", the address should start with rs.).`,
        code: 'TENANT_HOST_NOT_FOUND',
      });
    }
    ...
    if (!tenant.is_enabled) {
      return res.status(404).json({
        error: 'This workspace is disabled. Contact your administrator.',
