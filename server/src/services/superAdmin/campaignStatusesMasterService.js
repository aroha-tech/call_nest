...
const TABLE = 'campaign_statuses_master';
...
export async function findAll({ search = '', includeInactive = false, page = 1, limit = 10 } = {}) {
  ...
  const dataSQL = `SELECT * FROM ${TABLE} ${whereSQL} ORDER BY name ASC LIMIT ${limitNum} OFFSET ${offset}`;
...
export async function findAllActive() {
  return query(
    `SELECT id, name, code FROM ${TABLE} WHERE is_deleted = 0 AND is_active = 1 ORDER BY name ASC`
  );
}
...
export async function create(data, createdBy) {
  ...
  if (existing) {
    const err = new Error('Campaign status code already exists');
    err.status = 409;
    throw err;
  }

  await query(
    `INSERT INTO ${TABLE} (id, name, code, is_active, created_by, updated_by)
     VALUES (?, ?, ?, ?, ?, ?)`,
...
export async function update(id, data, updatedBy) {
  ...
  if (!row) {
    const err = new Error('Campaign status not found');
    err.status = 404;
    throw err;
  }
